...
class PatchChannelWidget(Gtk.Widget):
    ...
    def on_click(self, tgt, ev):
        ...
        child = self.app.patch_channels_tab.flowbox.get_child_at_index(
            self.channel-1)
    ...
    def do_draw(self, cr):
        ...
        area = (65, 600, 0, 60)
        a, b, c, d = area
        cr.arc(a + self.radius, c + self.radius,
               self.radius, 2*(math.pi/2), 3*(math.pi/2))
        cr.arc(b - self.radius, c + self.radius,
               self.radius, 3*(math.pi/2), 4*(math.pi/2))
        cr.arc(b - self.radius, d - self.radius,
               self.radius, 0*(math.pi/2), 1*(math.pi/2))
        cr.arc(a + self.radius, d - self.radius,
               self.radius, 1*(math.pi/2), 2*(math.pi/2))
        cr.close_path()
        cr.stroke()
