...
def save_midi_mapping(stream: Gio.FileOutputStream) -> None:
    ...
    for key, value in App().midi.messages.control_change.control_change.items():
        stream.write(bytes(f"$$MIDICC {key} {value[0]} {value[1]}\n", "utf8"))
    for key, value in App().midi.messages.pitchwheel.pitchwheel.items():
...
def _write_ascii(stream: Gio.FileOutputStream, text: str) -> None:
