...
class SequentialWidget(Gtk.Widget):
    ...
    def __init__(self, total_time, time_in, time_out,
                 delay_in, delay_out, wait, channel_time):
    ...
    def do_draw(self, cr):
        ...
        if self.wait > 0:
            ...
            if self.pos_xA or self.pos_xB:  # Red filter in fades
                cr.set_source_rgba(1, 0, 0, 0.1)
            cr.set_line_width(1)
            cr.rectangle(16, 40, (inter * self.wait),
        ...
        if self.delay_in:
            ...
            cr.move_to(16 + wait_x,
                       allocation.height - 32 - (len(self.channel_time) * 8))
            cr.line_to(16 + wait_x + (inter * self.delay_in),
                       allocation.height - 32 - (len(self.channel_time) * 8))
            cr.stroke()
        ...
        for channel in self.channel_time.keys():
            ...
            cr.move_to(16 + (inter * delay) + wait_x,
                       allocation.height - 8 - (self.ct_nb * 12))
            ...
            if next_level < old_level:
                ...
                if self.pos_xA > inter*delay + wait_x:
                    if self.pos_xA > (inter*delay)+(inter*time)+wait_x:
                        self.pos_xCT = ((inter * delay)
                                        + (inter * time)
                                        + wait_x)
                    else:
                        self.pos_xCT = self.pos_xA
                    cr.set_source_rgb(0.9, 0.6, 0.2)
                    cr.move_to(16 + self.pos_xCT,
                               allocation.height - 12 - (self.ct_nb * 12))
                    cr.line_to(16 + self.pos_xCT,
                               allocation.height - 4 - (self.ct_nb * 12))
                    cr.stroke()
                else:
                    cr.set_source_rgb(0.9, 0.6, 0.2)
                    cr.move_to(16 + (inter * delay) + wait_x,
                               allocation.height - 12 - (self.ct_nb * 12))
                    cr.line_to(16 + (inter * delay) + wait_x,
                               allocation.height - 4 - (self.ct_nb * 12))
                    cr.stroke()
            else:
                # In Crossfade
        ...
        cr.line_to((16 + wait_x + (inter * self.delay_out)
                    + (inter * self.time_out)),
                   allocation.height - 32 - (len(self.channel_time) * 8))
        cr.stroke()
        cr.move_to((16 + wait_x + (inter * self.delay_out)
                    + (inter * self.time_out)), 24)
        cr.line_to((16 + wait_x + (inter * self.delay_out)
                    + (inter * self.time_out)), allocation.height)
        cr.set_dash([8.0, 6.0])
        cr.stroke()
        cr.set_dash([])
        ...
        start_y = 40
        end_x = (16 + wait_x + (inter * self.delay_out)
                 + (inter * self.time_out))
        ...
        if ((not wait_x or self.pos_xA > wait_x)
                and (not self.delay_out
                     or self.pos_xA > (inter*self.delay_out)+wait_x)):
            x1 = start_x + self.pos_xA - wait_x - (inter*self.delay_out)
            y1 = (start_y + ((self.pos_xA - wait_x
        ...
        # draw In line
        cr.set_source_rgb(0.9, 0.5, 0.5)
        cr.move_to(16 + wait_x + (inter * self.delay_in),
                   allocation.height - 32 - (len(self.channel_time) * 8))
        cr.line_to((16 + wait_x + (inter * self.delay_in)
                    + (inter * self.time_in)), 40)
        cr.stroke()
        cr.move_to((16 + wait_x + (inter * self.delay_in)
                    + (inter * self.time_in)), 24)
        cr.line_to((16 + wait_x + (inter * self.delay_in)
                    + (inter * self.time_in)), allocation.height)
        cr.set_dash([8.0, 6.0])
        cr.stroke()
        cr.set_dash([])
        # draw an arrow at the end
        ...
        if ((not wait_x or self.pos_xB > wait_x)
                and (not self.delay_in
                     or self.pos_xB > ((inter*self.delay_in)+wait_x))):
            x1 = start_x + self.pos_xB - wait_x - (inter * self.delay_in)
            y1 = (start_y + ((self.pos_xB - wait_x
